...
from .movie_display import display_img, test_img_display
...
class training_class:
    ...
    def __init__(self, params_dict):
        ...
        self.test_datasize = (
            400
        )  # how many slices to be tested (use the first image in the folder by default)
    ...
    def prepare_file(self):
        ...
        # self.datasets_name = self.datasets_path.split("/")[-1]
        pth_name = (
            self.datasets_name + "_" + datetime.datetime.now().strftime("%Y%m%d%H%M")
        )
    ...
    def initialize_network(self):
        ...
        denoise_generator = Network_3D_Unet(
            in_channels=1, out_channels=1, f_maps=self.fmap, final_sigmoid=True
        )
    ...
    def train_preprocess_lessMemoryMulStacks(self):
        ...
        for im_name in list(os.walk(self.datasets_path, topdown=False))[-1][-1]:
            ...
            for x in range(
                0, int((self.whole_y - self.patch_y + self.gap_y) / self.gap_y)
            ):
                for y in range(
                    0, int((self.whole_x - self.patch_x + self.gap_x) / self.gap_x)
                ):
                    for z in range(
                        0, int((self.whole_t - patch_t2 + self.gap_t) / self.gap_t)
                    ):
                        single_coordinate = {
                            "init_h": 0,
                            "end_h": 0,
    ...
    def distribute_GPU(self):
        ...
        if torch.cuda.is_available():
            ...
            self.local_model = nn.DataParallel(
        ...
        for epoch in range(0, self.n_epochs):
            ...
            for iteration, (input, target) in enumerate(trainloader):
                ...
                if (iteration + 1) % (len(trainloader)) == 0:
                    ...
                    if (
                        self.visualize_images_per_epoch
                        | self.save_test_images_per_epoch
                    ):
                        print(
                            "Testing model of epoch {} on the first noisy file ----->".format(
                                epoch + 1
                            )
                        )
    ...
    def save_model(self, epoch, iteration):
        ...
        if isinstance(self.local_model, nn.DataParallel):
            torch.save(
                self.local_model.module.state_dict(), model_save_name
            )  # parallel
        else:
        ...
        if not self.colab_display:
            ...
            input = torch.randn(
                1, 1, self.patch_t, self.patch_x, self.patch_y, requires_grad=True
            ).cuda()
            torch.onnx.export(
                self.local_model.module,
                input,
                onnx_save_name,
                export_params=True,
    ...
    def test(self, train_epoch, train_iteration):
        ...
        (
            ...
            img_mean,
            input_data_type,
        ) = test_preprocess_chooseOne(self, img_id=0)
