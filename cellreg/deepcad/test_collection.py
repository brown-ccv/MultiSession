...
from .movie_display import test_img_display
...
class testing_class:
    ...
    def prepare_file(self):
        ...
        self.output_path = self.output_dir + "/" + self.denoise_model
    ...
    def set_params(self, params_dict):
        ...
        )  # check the number of GPU used for testing
        ...
        for pth_index in range(len(self.model_list)):
            ...
            if ".pth" in aaa:
                ...
                for N in range(len(self.img_list)):
                    ...
                    for iteration, (noise_patch, single_coordinate) in enumerate(
                        testloader
                    ):
                        ...
                        if postprocess_turn > 1:
                            for id in range(postprocess_turn):
                                ...
                                denoise_img[
                                    ...
                                    output_patch
                                    * (np.sum(raw_patch) / np.sum(output_patch)) ** 0.5
                                )
                                input_img[
                                    stack_start_s:stack_end_s,
                                    stack_start_h:stack_end_h,
                    ...
                    # Stitching finish
                    output_img = (
                        denoise_img.squeeze().astype(np.float32) * self.scale_factor
                    )
                    del denoise_img
