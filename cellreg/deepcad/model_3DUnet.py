...
class Noise2NoiseUNet3D(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, f_maps=16, num_groups=8, **kwargs):
        ...
        self.final_conv = SingleConv(
            f_maps[0], out_channels, kernel_size=1, order="cr", padding=0
        )
...
class TagsUNet3D(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.decoders = nn.ModuleList(
            [
                Decoder(
                    4 * init_channel_number + 8 * init_channel_number,
                    4 * init_channel_number,
                    conv_layer_order=conv_layer_order,
                    num_groups=num_groups,
                ),
                Decoder(
                    2 * init_channel_number + 4 * init_channel_number,
                    2 * init_channel_number,
                    conv_layer_order=conv_layer_order,
                    num_groups=num_groups,
                ),
                Decoder(
                    init_channel_number + 2 * init_channel_number,
                    init_channel_number,
                    conv_layer_order=conv_layer_order,
                    num_groups=num_groups,
                ),
    ...
    def forward(self, x):
        ...
        return [
            tag / torch.norm(tag, p=2, dim=1).detach().clamp(min=1e-8) for tag in tags
        ]
...
class DistanceTransformUNet3D(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, final_sigmoid, init_channel_number=32, **kwargs
    ):
...
class EndToEndDTUNet3D(nn.Module):
    def __init__(
        ...
        tags_out_channels,
        tags_output_heads,
        tags_init_channel_number,
        dt_in_channels,
        dt_out_channels,
        dt_final_sigmoid,
        dt_init_channel_number,
        tags_net_path=None,
        dt_net_path=None,
        **kwargs,
    ):
        ...
        self.dt_net = DistanceTransformUNet3D(
            dt_in_channels,
            dt_out_channels,
            dt_final_sigmoid,
