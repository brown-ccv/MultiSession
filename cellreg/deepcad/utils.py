...
def name2index(opt, input_name, num_h, num_w, num_s):
    ...
    if z_index == 0:
        ...
        patch_start_s = cut_s
        patch_end_s = opt.patch_t - cut_s
    return int(stack_start_w), int(stack_end_w), int(patch_start_w), int(
        patch_end_w
    ), int(stack_start_h), int(stack_end_h), int(patch_start_h), int(patch_end_h), int(
        stack_start_s
    ), int(stack_end_s), int(patch_start_s), int(patch_end_s)
...
def download_demo(download_filename):
    ...
    if not os.path.exists(save_img_folder + "/" + download_filename + ".tif"):
        if (
            (download_filename == "fish_localbrain")
            | (download_filename == "fish_wholebrain")
            | (download_filename == "drosophila")
        ):
            ...
            gdown.download(
                url_sum[0],
                save_img_folder + "/" + download_filename + ".tif",
                quiet=False,
                resume=True,
            )
    if not os.path.exists(save_pth_folder + "/" + "best_model.pth"):
        gdown.download(
            url_sum[1],
            save_pth_folder + "/" + "best_model.pth",
            quiet=False,
            resume=True,
        )
    if not os.path.exists(save_pth_folder + "/" + "best_model.yaml"):
        gdown.download(
            url_sum[2],
            save_pth_folder + "/" + "best_model.yaml",
            quiet=False,
            resume=True,
        )
